...
public class search extends AppCompatActivity {
    ...
    { // create table: results
        ...
        try
        {
            ...
            // create table
            db.execSQL("create table if not exists heat ("
                    + " recID integer PRIMARY KEY autoincrement, "
                    + " Eartag  TEXT, " + " Name TEXT, " + " Date TEXT);  ");
            // commit your changes
    ...
    private void openDatabase()
    {
        try
        {
            ...
            String SDcardPath = "data/data/com.example.marti.farmapp/databases";
    ...
    private class Query_all_Database extends AsyncTask<Void, Void, Void> {
        ...
        protected void onPostExecute(final Void unused)
        {
            ...
            for (int i =0; i< heat_data.size();i++)
            {
                results.append("***********************************************\n");
                results.append("Eartag: " + heat_data.get(i).getEartag() +"\n");
                results.append("Feed Name: " + heat_data.get(i).getBullID() +"\n");
                results.append("Feed Amount: " + heat_data.get(i).getHeatDates() +"\n");
                results.append("***********************************************\n\n");
            }
            results.append("Time Taken: "+ (System.currentTimeMillis() -startingMillis) +"msec\n\n");
        }
    ...
    /*******************************************************************************************/
    private void QueryDatabaseheat()
    {

        try
        {
            String mySQL = "select * from heat where Eartag LIKE '%"+name+"%'";
            Cursor c1 = db.rawQuery(mySQL,null);
            heat_data = get_All_The_Heat(c1);
        }
        catch (Exception e)
        {
            Log.e("\n#Error: ", "QueryDatabase(): " + e.getMessage());
    ...
    private List<heatData> get_All_The_Heat(Cursor cursor)
    {
        cursor.moveToPosition(-1); //reset cursor's top
        ...
        try
        {
            ...
            while (cursor.moveToNext())
            {
                for (int i = 0; i < cursor.getColumnCount(); i++)
                {
                    lah = cursor.getString(1);
                    irh = cursor.getString(2);
                    lph = cursor.getString(3);


                }
                data.add(new heatData(lah, irh, lph));
            }
        }
        catch (Exception e)
        {
    ...
    public void RetrieveFromHeat (View view)
    {
        new Query_heat_Database().execute();
    }
    private class Query_heat_Database extends AsyncTask<Void, Void, Void> {
